...
class SetupScreen(tk.Frame):
    ...
    def _build(self):
        ...
        hdr = card_frame(self, pady=20, padx=30)
        hdr.grid(row=0, column=0, sticky="ew", padx=20, pady=(30, 10))
        ...
        form = card_frame(self, padx=30, pady=25)
        ...
        for i, (label, var_name, kind, opts) in enumerate(fields):
            styled_label(form, label, size=10, color=C["pink_mid"]).grid(
                row=i, column=0, sticky="w", pady=7, padx=(0, 14))
            if kind == "entry":
                var = tk.StringVar()
                tk.Entry(form, textvariable=var, width=28, **entry_kw).grid(
                    row=i, column=1, sticky="ew", pady=7)
            else:
                var = tk.StringVar(value=opts[1])
        ...
        self._vars["name_var"].set("")
        self._vars["loc_var"].set("Dhaka, BD")
        self._vars["temp_var"].set("34.5")
        self._vars["humid_var"].set("82")

        styled_button(form, "Start Tracking  →", self._submit,
                      big=True, width=22).grid(
            row=len(fields), column=0, columnspan=2, pady=(20, 0))
    ...
    def _submit(self):
        ...
        user    = User(name=name, trimester=trim_map[trim_s],
                       activity_level=act_map[act_s], location=loc)
...
class DashboardScreen(tk.Frame):
    ...
    def _build(self):
        ...
        self._build_tips_card(right)       # ← NEW: tips & facts panel
    ...
    def _build_tips_card(self, parent):
        ...
        self._tips_panel.grid(row=3, column=0, sticky="ew", pady=(0, 10))
...
class HydrationApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Mum Mum Help - A daily companion for Moms")
